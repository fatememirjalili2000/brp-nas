...
class MetricsCollector:
    ...
    def compute_regression_metrics(self, targets, predictions, leeways=[0.01, 0.05, 0.1, 0.2]):
        ...
        # **FIXED: Robust R² calculation to prevent negative values**
        ss_res = np.sum((targets_np - predictions_np) ** 2)
        ss_tot = np.sum((targets_np - np.mean(targets_np)) ** 2)
        
        if ss_tot < 1e-12:  # All targets are essentially the same
            r2 = 1.0 if ss_res < 1e-12 else 0.0
        else:
            r2 = 1 - (ss_res / ss_tot)
            # Clamp R² to reasonable range to prevent extreme negative values
    ...
    def get_final_results(self, cfg, model_name, total_training_time):
        ...
        total_inference_time = sum(self.epoch_times) if self.epoch_times else 0.0
    ...
    def save_results(self, results, outdir, model_name, exp_name=None):
        ...
        filename = f"results_{model_name}"
        if exp_name:
    ...
    def create_charts(self, results, outdir, model_name, exp_name=None):
        ...
        try:
            ...
            plt.tight_layout()
            loss_chart_path = charts_dir / f"training_metrics_{model_name}{f'_{exp_name}' if exp_name else ''}.png"
            ...
            if self.val_accuracies:
                ...
                accuracy_chart_path = charts_dir / f"accuracy_metrics_{model_name}{f'_{exp_name}' if exp_name else ''}.png"
            ...
            if self.epoch_times and self.memory_usage and self.cpu_usage:
                ...
                plt.tight_layout()
                system_chart_path = charts_dir / f"system_metrics_{model_name}{f'_{exp_name}' if exp_name else ''}.png"
            ...
            if self.best_val_targets and self.best_val_predictions:
                ...
                scatter_chart_path = charts_dir / f"predictions_scatter_{model_name}{f'_{exp_name}' if exp_name else ''}.png"
